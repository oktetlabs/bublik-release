...
- **dashboard:** add tv mode for dashboard for fullscreen viewing ([65ba623](#))
- **auth:** add authentication ([97da705](#))
- **auth:** add user profile page ([2fae3af](#))
- **import:** integrate import logs inside bublik ([a9b7854](#))
- **history:** sync client filter state with search params ([3583a23](#))
...
- **dashboard** make dashboard errors to display properly ([bd84bb0](#))

#### 🐛 Bug Fix

- **dashboard** failing recurcively on dashboard fetch error ([9956060](#))
- **dashboard** not refetching dashboard when run is marked as compromised ([9d9b4bf](#))
- **history** changing date in form and closing not picking correct date ([fdab344](#))
- **history** changing mode when no page exists returns 404 ([5d73e3d](#))

#### ♻ Code Refactoring

- **dashboard** refactor dashboard table as well as search params ([c65cf37](#))

#### 👷‍ Build System

- **ci** update ci/cd actions to latest versions ([de463ff](#))
